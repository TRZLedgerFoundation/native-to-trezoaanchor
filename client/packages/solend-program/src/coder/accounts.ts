...
export class SolendProgramAccountsCoder<A extends string = string>
  implements AccountsCoder
{
  ...
  public async encode<T = any>(accountName: A, account: T): Promise<Buffer> {
    switch (accountName) {
      case "obligation": {
        const buffer = Buffer.alloc(1300);
...
const LENDING_MARKET_LAYOUT: any = B.struct([
  ...
  B.publicKey("switchboardOracleProgramId"),
  B.blob(128, "padding"),
]);
...
const RESERVE_LAYOUT: any = B.struct([
  ...
  B.struct(
    [
      ...
      B.publicKey("pythOraclePubkey"),
      B.publicKey("switchboardOraclePubkey"),
      B.u64("availableAmount"),
      B.decimal("borrowedAmountWads"),
      B.decimal("cumulativeBorrowRateWads"),
      B.decimal("accumulatedProtocolFeesWads"),
